...
#### Dot Product
To get the dot product of $`\vec{u}`$ and $`\vec{v}`$ ($`\vec{u} \cdot \vec{v}`$) you just need to type in `u*v`, like a regular multiplication. Raw vectors are also accepted `(2,-1,0)*(1,1,-2)`
#### Cross product
